function isotp_com_test()
    ...
    % On Linux, your original logic expects 'isotp_mex_linux'
    % If the user is on Windows, we'll skip that and use CANInterface_Windows instead.
    if ~ispc
        ...
        % If Windows, optionally we can do a quick check for CANInterface_Windows
        % (not strictly required, but good to confirm)
        if ~exist('windows\CANInterface_Windows.m','file')
            error('CANInterface_Windows.m not found on the MATLAB path. Cannot run on Windows.');
        end
    end
    ...
    for k = 1:numel(dataIDBundles)
        ...
        requestVec = uint8([0x03,0x22, bitshift(dataID, -8), bitand(dataID, 255)]);
        ...
        if ispc
            ...
            addpath(libraryPath);
            % Use CANInterface_Windows to handle ISO-TP (like in your "tranceiveCAN" approach)
            %resp = CANInterface_Windows('tranceiveCAN', canInterface, rxIdHex, txIdHex, requestVec);
            %resp = CANInterface_Windows('tranceiveCAN', 'PEAK-System', [], [], '18DAF101', '18DA01F1', uint8([0x22, 0xF1, 0x90]));
            resp = CANInterface_Windows('tranceiveCAN', 'PEAK-System', [], '18DAF101', '18DA01F1', requestVec);
        else
            % == LINUX or OTHER ==
            % Use your original isotp_mex_linux
    ...
    disp('Finished reading Data IDs via isoTpRequestMex / CANInterface_Windows.');
end
